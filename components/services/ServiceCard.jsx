...
const ServiceCard = ({ serversData, index }) => {
  return (
    ...
    >
      ...
      <div className="mb-6">
        <h5
          className={`mb-2 font-bold tracking-wide text-center text-md md:text-md lg:text-lg xl:text-lg 2xl:text-2xl ${index === 1 ? "text-white group-hover:text-secondary" : "text-secondary group-hover:text-secondary"}`}
        >
          {serversData?.title}
        </h5>
        <p
          className={`font-normal text-center line-clamp-2 ${index === 1 ? "text-white group-hover:text-secondary" : "text-gray-700 group-hover:text-secondary"}`}
        >
